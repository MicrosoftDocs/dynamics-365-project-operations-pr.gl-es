...
| Recurso reservable | O nome do subcontratista cuxo tempo se está a facturar. A selección dun recurso reservable é opcional. | Se este campo se deixa en branco, o xestor do proxecto pode facer coincidir a liña de factura do provedor coa hora rexistrada por calquera recurso que pertenza ao provedor na liña de factura do provedor. |
...
| Prezo por unidade | O prezo unitario predeterminado utiliza a combinación de **Papel** e **Unidade** valores da lista de prezos do proxecto que é aplicable á data de transacción da liña de factura do provedor. | Se o prezo da lista de prezos do proxecto aplicable está configurado nunha unidade que difire da unidade da liña de factura do provedor, o sistema utiliza a conversión unitaria para calcular o prezo por unidade. |
