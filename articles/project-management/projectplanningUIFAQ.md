...
Project Operations require que se activen as cookies de terceiros para representar a estrutura de subdivisión do traballo. Cando as cookies de terceiros non estean activadas, en lugar de ver tarefas, verá unha páxina en branco cando seleccione o **Tarefas** ficha na **Proxecto** páxina.
...
2. Identifica o tipo de produto que estás a usar. Este valor úsase cando se establece o extremo PEX. Ao recuperalo, o tipo de produto xa está definido no extremo PEX. Manteña ese valor.
...
No teu navegador, abre unha nova pestana, vai a project.microsoft.com e inicia sesión co rol de usuario que estás a usar para acceder ás Operacións do proxecto. É importante que só un usuario inicie sesión nun produto de Microsoft no navegador. A mensaxe de erro "login.microsoftonline.com negouse a conectarse" ocorre con máis frecuencia cando máis dun usuario está conectado, como se mostra na seguinte ilustración.

![Escolle unha páxina de inicio de sesión na conta que mostre que dous usuarios iniciaron sesión.](media/MULTIPLE_USERS_LOGGED_IN.png)
...
Para os efectos da autenticación, as ventás emerxentes deben estar activadas para que se cargue a grade Tarefa. Se as ventás emerxentes non están activadas, a pantalla quedará atascada no indicador de progreso de carga. O seguinte gráfico mostra o URL cunha etiqueta emerxente bloqueada na barra de enderezos, o que fai que o xirator se atasque ao tentar cargar a páxina. 
...
Project Operations depende dun servizo de programación externo. O servizo require que un usuario teña asignados varios roles que lle permitan ler e escribir en entidades relacionadas coa WBS. Estas entidades inclúen tarefas do proxecto, atribucións de recursos e dependencias de tarefas. Se un usuario non pode renderizar o WBS cando navega ao **Tarefas** ficha, probablemente sexa porque **Proxecto** para **Operacións do proxecto** non se habilitou. Un usuario pode recibir un erro de rol de seguranza ou un erro relacionado cunha denegación de acceso.
